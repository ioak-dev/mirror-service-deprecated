...
class TransientModel:
    ...
    def load_model(self, tenant):
        ...
        all_models[tenant] = pickle.load(open(trained_models_dir + '/model.sav', 'rb'))
        all_vectorizers[tenant] = pickle.load(open(trained_models_dir + '/vectorizer.sav', 'rb'))
        all_labels[tenant] = pickle.load(open(trained_models_dir + '/label.sav', 'rb'))
    ...
    def train(self, tenant):
        ...
        vectorizer=TfidfVectorizer(sublinear_tf=True, min_df=1, norm='l2', ngram_range=(1, 2), stop_words='english', use_idf=True, max_df=1.0, max_features=2000)
        ...
        filename = trained_models_dir + '/model.sav'
        vec_filename = trained_models_dir + '/vectorizer.sav'
        lab_filename = trained_models_dir + '/label.sav'
        pickle.dump(classifier, open(filename, 'wb'))
        pickle.dump(vectorizer, open(vec_filename, 'wb'))
        pickle.dump(dict((v, k) for k, v in computed_labels.items()), open(lab_filename, 'wb'))
    ...
    def prediction(self, tenant, data):
        ...
        return (np.argsort(outcome[0]), all_labels[tenant])
        #pass
